...
def main():
    ...
    log_model_with_mlflow(
        rf_model, X_test, y_test, "RandomForestClassifier", experiment_id, output_dir
    )

    lr_model = train_logistic_regression(X_train, y_train)
    log_model_with_mlflow(
        lr_model, X_test, y_test, "LogisticRegression", experiment_id, output_dir
    )

    dt_model = train_decision_tree(X_train, y_train)
    log_model_with_mlflow(
        dt_model, X_test, y_test, "DecisionTreeClassifier", experiment_id, output_dir
    )
